...
use fogcore::TileRendererPremium2;
...
fn benchmark_tile_rendering_premium2(c: &mut Criterion) {
    let fogmap = load_tracks_map_folder("static/tiles");
    let bg_color = image::Rgba([100, 0, 100, 255]);
    let fg_color = image::Rgba([0, 0, 0, 0]);
    let lng = 114.1;
    ...
    let mut group = c.benchmark_group("tile_rendering_premium2");
    ...
    for zoom in [1, 5, 10, 15, 20] {
        for &resolution in &[
            ...
            let renderer = TileRendererPremium2::new(resolution);
...
criterion_group!(
    ...
    benchmark_tile_rendering_premium2
);
